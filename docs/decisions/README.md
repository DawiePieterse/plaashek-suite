...
16. ~~Kudde: re-evaluate against a real livestock farm, per ADR 0005's own trigger~~ — [ADR 0014](0014-kudde-bekfontein.md): Bekfontein now runs cattle — Kudde gets a build slot, scoped to individual-animal identity (farm-typed tag, ADR 0011's pattern), per-animal movements written in a batch, and no breeding/mating-group model
